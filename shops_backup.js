    ...
    modal.innerHTML = `
        <div class="modal" style="max-width: 800px; width: 90%; max-height: 90vh;">
            ...
            <div class="modal-body" style="overflow-y: auto; max-height: 70vh;">
                <div style="margin-bottom: 1rem; padding: 1rem; background: rgba(182, 103, 255, 0.1); border: 1px solid var(--accent); border-radius: 8px;">
                    <h4 style="color: var(--accent); margin-bottom: 0.5rem;">Памятка:</h4>
                ...
                <div style="display: grid; gap: 1rem;">
                    ${HOUSING_OPTIONS.map(housing => {
                        ...
                        return `
                            <div style="background: rgba(0,0,0,0.3); border: 1px solid var(--border); border-radius: 8px; padding: 1rem;">
                                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                                    <div>
                                        <h4 style="color: var(--text); margin: 0 0 0.25rem 0; font-size: 1.1rem;">${housing.name}</h4>
                                        <div style="color: var(--muted); font-size: 0.8rem; margin-bottom: 0.5rem;">${housing.area}</div>
                                    </div>
                                    <div style="text-align: right;">
                                        ${housing.isDefault ? `
                                            <div style="color: var(--accent); font-size: 0.9rem; font-weight: 600;">Бесплатно</div>
                                        ` : ''}
                                        ${housing.rentPrice !== null && !housing.isDefault ? `
                                            <div style="color: var(--warning); font-size: 0.9rem; font-weight: 600;">Аренда: ${housing.rentPrice.toLocaleString()} уе/мес</div>
                                        ` : ''}
                                        ${housing.buyPrice !== null && !housing.isDefault ? `
                                            <div style="color: var(--success); font-size: 0.9rem; font-weight: 600;">Покупка: ${housing.buyPrice.toLocaleString()} уе</div>
                                        ` : ''}
                                    </div>
                                </div>
                                <p style="color: var(--muted); font-size: 0.85rem; line-height: 1.4; margin-bottom: 1rem;">${housing.description}</p>
                                <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                    ${housing.isDefault && !isOwned ? `
                                        <button class="pill-button primary-button" onclick="moveToDefaultHousing()" style="font-size: 0.8rem; padding: 0.3rem 0.6rem;">Заселиться</button>
                                    ` : ''}
                                    ${housing.isDefault && isOwned ? `
                                        <div style="color: var(--success); font-size: 0.8rem; padding: 0.3rem 0.6rem; font-weight: 600;">✓ В собственности</div>
                                    ` : ''}
                                    ${!housing.isDefault && housing.rentPrice !== null ? `
                                        <button class="pill-button warning-button" onclick="rentHousing('${housing.id}')" style="font-size: 0.8rem; padding: 0.3rem 0.6rem;">Арендовать</button>
                                    ` : ''}
                                    ${!housing.isDefault && housing.buyPrice !== null ? `
                                        <button class="pill-button success-button" onclick="buyHousing('${housing.id}')" style="font-size: 0.8rem; padding: 0.3rem 0.6rem;">Купить</button>
                                    ` : ''}
                                </div>
                            </div>
                        `;
                ...
                <div style="display: grid; gap: 1rem;">
                    ${COMMERCIAL_PROPERTY_OPTIONS.map(property => `
                        <div style="background: rgba(0,0,0,0.3); border: 1px solid var(--border); border-radius: 8px; padding: 1rem;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                                <div>
                                    <h4 style="color: var(--text); margin: 0 0 0.25rem 0; font-size: 1.1rem;">${property.name}</h4>
                                    <div style="color: var(--muted); font-size: 0.8rem; margin-bottom: 0.5rem;">${property.area}</div>
                                </div>
                                <div style="text-align: right;">
                                    <div style="color: var(--warning); font-size: 0.9rem; font-weight: 600;">Аренда: ${property.rentPrice.toLocaleString()} уе/сутки</div>
                                </div>
                            </div>
                            <p style="color: var(--muted); font-size: 0.85rem; line-height: 1.4; margin-bottom: 1rem;">${property.description}</p>
                            <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                <button class="pill-button warning-button" onclick="rentCommercialProperty('${property.id}')" style="font-size: 0.8rem; padding: 0.3rem 0.6rem;">Арендовать</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
    ...
    if (currentMoney < housing.buyPrice) {
        showModal('Недостаточно средств', `
            <div style="text-align: center; padding: 1rem;">
                <p style="color: var(--danger); font-size: 1.1rem; margin-bottom: 1rem;">Недостаточно денег!</p>
                <p style="color: var(--muted);">Нужно: ${housing.buyPrice.toLocaleString()} уе</p>
                <p style="color: var(--muted);">У вас: ${currentMoney.toLocaleString()} уе</p>
                <p style="color: var(--muted);">Не хватает: ${(housing.buyPrice - currentMoney).toLocaleString()} уе</p>
            </div>
        `);
        return;
    }
...
function rentHousing(housingId) {
    ...
    if (currentMoney < housing.rentPrice) {
        showModal('Недостаточно средств', `
            <div style="text-align: center; padding: 1rem;">
                <p style="color: var(--danger); font-size: 1.1rem; margin-bottom: 1rem;">Недостаточно денег!</p>
                <p style="color: var(--muted);">Нужно: ${housing.rentPrice.toLocaleString()} уе/мес</p>
                <p style="color: var(--muted);">У вас: ${currentMoney.toLocaleString()} уе</p>
                <p style="color: var(--muted);">Не хватает: ${(housing.rentPrice - currentMoney).toLocaleString()} уе</p>
            </div>
        `);
        return;
    }
...
function rentCommercialProperty(propertyId) {
    ...
    if (currentMoney < property.rentPrice) {
        showModal('Недостаточно средств', `
            <div style="text-align: center; padding: 1rem;">
                <p style="color: var(--danger); font-size: 1.1rem; margin-bottom: 1rem;">Недостаточно денег!</p>
                <p style="color: var(--muted);">Нужно: ${property.rentPrice.toLocaleString()} уе/сутки</p>
                <p style="color: var(--muted);">У вас: ${currentMoney.toLocaleString()} уе</p>
                <p style="color: var(--muted);">Не хватает: ${(property.rentPrice - currentMoney).toLocaleString()} уе</p>
            </div>
        `);
        return;
    }
...
function removeHousing(index) {
    ...
    showModal('Удалить жилье', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--text); font-size: 1.1rem; margin-bottom: 1rem;">Удалить "${housing.name}"?</p>
            <p style="color: var(--muted);">Это действие нельзя отменить.</p>
...
function removeCommercialProperty(index) {
    ...
    showModal('Удалить коммерческую недвижимость', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--text); font-size: 1.1rem; margin-bottom: 1rem;">Удалить "${property.name}"?</p>
            <p style="color: var(--muted);">Это действие нельзя отменить.</p>
            <div style="display: flex; gap: 0.5rem; justify-content: center; margin-top: 1rem;">
                        ...
                        <div style="background: rgba(182, 103, 255, 0.1); border: 1px solid var(--accent); border-radius: 8px; padding: 1rem;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                <div style="flex: 1;">
                                    ...
                                    <div class="vehicle-price-display" style="color: var(--success); font-weight: 600; font-size: 1rem;" data-original-price="${vehicle.price}">
                                        ${(() => {
                                            ...
                                            return hasDiscount ? 
                                                `Цена: <span style="text-decoration: line-through; color: var(--muted);">${vehicle.price}</span> <span style="color: var(--success); font-weight: 600;">${finalPrice}</span> <span style="color: var(--success); font-size: 0.75rem;">(-${Math.round(totalDiscount * 100)}%)</span> уе` :
...
// Заглушки для функций магазинов (будут переопределены в deck.js)
function showProgramShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина программ загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showDeckShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина дек загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showDeckGearShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина снаряжения дек загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showDeckCollection() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция коллекции дек загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showGearShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина снаряжения загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function pickupGear() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция подбора снаряжения загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showImplantShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина имплантов загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showImplantPartsShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина частей имплантов загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showImplantsManagement() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция управления имплантами загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}
...
function showWeaponShop() {
    showModal('Загрузка...', `
        <div style="text-align: center; padding: 1rem;">
            <p style="color: var(--accent); font-size: 1.1rem; margin-bottom: 1rem;">🔄 Загружается...</p>
            <p style="color: var(--muted);">Функция магазина оружия загружается. Попробуйте еще раз через несколько секунд.</p>
        </div>
    `);
}

console.log('✅ shops.js загружен - все магазины готовы (включая заглушки)');